...
<div class="title-bar top_title"><p>{{Title}}</p></div> <div class="title-bar"><p>numeric validation  {{validatorType}}  will check whether the value entered is a valid numberic value or not.The validation can be set according to requirement, it can be either decimal,negative number or positive number.</p></div>
