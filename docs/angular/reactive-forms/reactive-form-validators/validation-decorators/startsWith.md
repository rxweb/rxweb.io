...
<div class="title-bar top_title"><p>{{Title}}</p></div> <div class="title-bar"><p>startsWith validation  {{validatorType}}  allows user to enter the input which starts with particular value.</p></div>
...
<div component="app-code" key="startsWith-isRestrictExample-model"></div> 
<div component="app-example-runner" ref-component="app-startsWith-isRestrict" title="startsWith {{validatorType}} with isRestrict" key="isRestrict"></div>
...
This Dynamic startsWith example is executed on the basis of json passed in the formBuilderConfiguration which comes under `RxFormBuilder` of reactive-form-validators. `conditionalExpression` with function would not be applied in dynamic startsWith example. This example will fulfil the requirement of our last point.
