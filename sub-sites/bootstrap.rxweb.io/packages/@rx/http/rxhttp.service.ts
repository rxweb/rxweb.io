...
export class RxHttp {
    ...
    post<T>(url: string | AuthorizeApi, jObject: Object, showToast?: boolean): Observable<T> {
        ...
        if (options.options)
            return this.http.post(
                options.uri, JSON.stringify(this.getJObject(jObject)), options.options
            ).map(
                (response: Response) => {
                    return this.requestResponse<T>(response, 'POST', showToast)
                }
    ...
    private getJsonObject(value: any) {
        var jObject = {};
        for (var col in value) {
            if (value[col] instanceof Array) {
                jObject[col] = [];
                for (var i = 0; i < value[col].length; i++) {
                    jObject[col].push(this.getJsonObject(value[col][i]));
                }
            }
            else if (value[col] instanceof Date)
                jObject[col] = `${value[col].getFullYear()}/${value[col].getMonth() + 1}/${value[col].getDate()}`
            else
                jObject[col] = value[col];

        }
        return jObject;
    }

    private getJObject(value: any) {
        var jObject: any = {};
        if (value) {
            if (value && value[0] == undefined) {
                jObject = this.getJsonObject(value);
            } else {
                jObject = [];
                for (var i = 0; i < value.length; i++) {
                    jObject.push(this.getJsonObject(value[i]));
                }
            }
        }
        return jObject;
    ...
    put<T>(url: string | AuthorizeApi, jObject: Object, showToast?: boolean): Observable<T> {
        ...
        if (options.options) {
            ...
            return this.http.put(
                finalUri, JSON.stringify(this.getJObject(jObject)), options.options
            ).map(
                (response: Response) => {
