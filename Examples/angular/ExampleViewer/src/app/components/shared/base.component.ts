...
export abstract class BaseComponentProvider implements OnDestroy {
    ...
    getParams(element:any,key:string,type?:string){

        let dynamicsplitedArray = location.pathname.split('/');
        let refComponentString = "app-" +  dynamicsplitedArray[2] + "-" + key; 
        let keyName = key;
      //  //let keyName = element.getAttribute("key");
        ...
        if(refComponentString){
            ...
            let refComponent = refComponentString.split('-');
                ...
                else if (dynamicsplitedArray[1] == "reactive-dynamic-forms" || dynamicsplitedArray[1] == "rxweb-storage" || dynamicsplitedArray[1] == "rxweb-generics" || dynamicsplitedArray[1] == "rxweb-http" || dynamicsplitedArray[1] == "rxweb-sanitizers")
