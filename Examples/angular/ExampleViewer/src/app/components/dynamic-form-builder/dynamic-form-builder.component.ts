...
export class DynamicFormBuilderComponent extends DynamicFormBuilderDomain implements OnInit {
    controlTypeList = ["checkbox", "select", "multiselect", "checkboxlist", "radio", "singlefile", "multiplefile", "textbox", "textarea"];
            ...
            if (jsonObject.hasOwnProperty(key)) {
                ...
                if (typeof jsonObject[key] == "object") {
                    ...
                    if (jsonObject[key].length > 0) {
                        ...
                        // if(isExist.count() == 0 && !isArray)
                        //     modelList.push(model)
                        if(typeof jsonObject[key][0] == "string"){
                            parentKey = "";
                            propertyModel.propertyName = key;
                            propertyModel.propertyValue = jsonObject[key]
                            model.annotationType = "@prop()"
    ...
    checkboxListAdd(element: any, propertyName: string) {
        var value = this.builderGroup.controls[propertyName].value;
        if (value == null)
            value = [];
        else
        ...
        this.builderGroup.controls[propertyName].patchValue(value)
    }
    radioButtonSetValue(element:any,propertyName:string){
        this.builderGroup.controls[propertyName].patchValue(element.value)
    }
    setContent(params) {
        let JsonObj = {}
        ...
        for (let i = 0; i < propertiesLength; i++) {
            ...
            var dropdownItems = this.dynamicFormBuilderFormGroup.controls['keyList']['controls'][i]['controls']['dropdownSource'].value
            if (dropdownItems == null)
                this.dynamicFormBuilderFormGroup.controls['keyList']['controls'][i]['controls']['dropdownSourceList'].patchValue([])
            else {
                if (this.dynamicFormBuilderFormGroup.controls['keyList']['controls'][i]['controls']['controlName'].value == 'multiselect') {
                    let items: any[] = [];
                    dropdownItems.split(',').forEach(element => {
                        items.push({ "keyName": element, "keyValue": element })
                    });
                    this.dynamicFormBuilderFormGroup.controls['keyList']['controls'][i]['controls']['dropdownSourceList'].patchValue(items)
                }
                else {
                    this.dynamicFormBuilderFormGroup.controls['keyList']['controls'][i]['controls']['dropdownSourceList'].patchValue(dropdownItems.split(','))
                }
            }
                ...
                for (let j = 0; j < validatorsLength; j++) {
                    ...
                    if (!isHide) {
                        ...
                        for (let k = 0; k < parameterItemsLength; k++) {
                            ...
                            let keyValue = keyLabel == "matchValues" ?  ((parameterItems[k].keyValue==null) ? parameterItems[k].keyValue : parameterItems[k].keyValue.split(",")) : parameterItems[k].keyValue;
                            if (keyValue !== "") {
                                if (k == 0)
