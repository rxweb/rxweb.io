...
@Component({
  ...
  templateUrl: './one-of-async-complete.component.html'
})
export class OneOfAsyncCompleteValidatorComponent implements OnInit {
  ...
  ngOnInit() {
    this.employeeInfoFormGroup = this.formBuilder.group({
      ...
      qualifications: ['',[], RxwebValidators.oneOfAsync({validatorConfig:of({ matchValues: ["Secondary", "Senior Secondary", "B.Tech", "M.Tech", "B.C.A.", "M.C.A."], conditionalExpression: (x, y) => x.department == 'DotNet' })})],
      skills: ['',[], RxwebValidators.oneOfAsync({validatorConfig:of({ matchValues: ["MVC", "AngularJS", "Angular 5", "C#", "Web Api", "SQL Server"], conditionalExpression: "x => x.department =='DotNet'" })})],
      ...
      languages: ['',[], RxwebValidators.oneOfAsync({validatorConfig:of({ matchValues: ["English", "Spanish", "French"], messageKey: 'oneOfMessageKey' })})]
    });
  }
